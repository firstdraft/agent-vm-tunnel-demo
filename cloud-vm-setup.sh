...
install_portable_ruby() {
  [ "$(uname -m)" = x86_64 ] || { echo "cloud-vm-setup: ruby-builder fallback supports x86_64 only" >&2; return 1; }
...
use_exact_ruby || { install_portable_ruby && use_exact_ruby; } || {
  echo "cloud-vm-setup: Ruby $RUBY_VERSION is unavailable from PATH, asdf, mise, or the portable fallback." >&2
  exit 1
}
